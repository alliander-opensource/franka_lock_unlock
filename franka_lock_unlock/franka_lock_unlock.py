...
class FrankaLockUnlock:
    ...
    def _request_token(self, physically=False):
        ...
        token_request = self._session.post(
            urljoin(
                self._hostname,
                f'/admin/api/control-token/request{"?force" if physically else ""}',
            ),
    ...
    def run(
        ...
        assert (
            not fci or persistent
        ), "Activating FCI without persistence is not possible."
        assert (
            not home or unlock
        ), "Homing the gripper without unlocking is not possible."
        self._login()
        try:
            assert (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "hostname",
        help='The Franka Desk IP address or hostname, for example "1.2.3.4".',
    )
    ...
    parser.add_argument(
        ...
        "--unlock",
        action="store_true",
        help="Unlock the brakes. Otherwise, lock them.",
    )
    parser.add_argument(
        "-l", "--relock", action="store_true", help="Relock the brakes on exit."
    )
    parser.add_argument(
        "-w",
        "--wait",
    ...
    assert (
        not args.relock or args.unlock
    ), "Relocking without prior unlocking is not possible."
    assert (
